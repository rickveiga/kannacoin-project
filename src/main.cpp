...
#if defined(NDEBUG)
# error "Kannacoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x52a21de71a0920d20c7f237b53ceaa654a3a27db5816e9710caf660353b0929a");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Kannacoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Kannacoin Signed Message:\n";
...
{
    // Kannacoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 250000); // Kannacoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Kannacoin: 3.5 days
static const int64 nTargetSpacing = 5 * 60; // Kannacoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Kannacoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Kannacoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xd92005a8433348c16cd5eac3fba3aa689adf425a14b99d2b47a974d54c5f4a18");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Nationwide legalization of Cannabis products";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 40 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04d2732e109b674b3096c2e287f70d4f173560e0aa05acacc49c70560e7bde625e27afbf594cd6949bbaaffdc71ba0020582d6f1d9e78471853c06f362551d1fa7") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1544045470;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085427531;

        if (fTestNet)
        {
            block.nTime    = 1544045432;
            block.nNonce   = 386120864;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("a4448aa1bf2a33bbfb1d505f87c1d5c2153ea15c646dd3bb0b0f5b407d5978ad"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xcb, 0xb1, 0xdc }; // Kannacoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("KannacoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("KannacoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static KannacoinMiner(CWallet *pwallet)
{
    printf("KannacoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("kannacoin-miner");
    ...
    try { loop {
        ...
        printf("Running KannacoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("KannacoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&KannacoinMiner, pwallet));
}
